...
public class QueryControllerPerformanceTest {
    ...
    @Test
    public void shouldReturnNotFoundForRemovedLegacyReadApis() throws Exception {
        mockMvc.perform(get("/api/plans/11"))
                .andExpect(status().isNotFound());
        mockMvc.perform(get("/api/plans/11/tasks"))
                .andExpect(status().isNotFound());
        mockMvc.perform(get("/api/tasks/101/executions"))
                .andExpect(status().isNotFound());
    }

    @Test
    public void shouldExposePlanStatusInTaskDetailWithoutLegacyPlanApi() throws Exception {
        AgentPlanEntity plan = new AgentPlanEntity();
        plan.setId(11L);
        plan.setStatus(com.getoffer.types.enums.PlanStatusEnum.RUNNING);
        ...
        when(agentTaskRepository.findById(101L)).thenReturn(task1);
        when(taskExecutionRepository.findLatestExecutionTimeByTaskIds(anyList()))
                .thenReturn(Map.of(101L, 180L));

        mockMvc.perform(get("/api/tasks/101"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value("0000"))
                .andExpect(jsonPath("$.data.taskId").value(101L))
                .andExpect(jsonPath("$.data.planStatus").value("RUNNING"))
                .andExpect(jsonPath("$.data.latestExecutionTimeMs").value(180L));

        verify(taskExecutionRepository, times(1)).findLatestExecutionTimeByTaskIds(anyList());
    }
