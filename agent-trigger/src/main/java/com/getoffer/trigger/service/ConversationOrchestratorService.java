...
/**
 * 会话编排服务：统一处理会话创建/复用、回合创建、规划触发与失败兜底。
 */
@Slf4j
@Service
public class ConversationOrchestratorService {
    ...
    public ConversationSubmitResult submitMessage(ChatMessageSubmitRequestV3DTO request) {
        ...
        try {
            savedTurn = createTurn(session.getId(), userMessage);
    ...
    private AgentSessionEntity resolveOrCreateSession(ChatMessageSubmitRequestV3DTO request) {
        ...
        entity.setAgentKey(resolvedAgentKey);
        entity.setScenario(scenario);
        entity.setMetaInfo(mergeMetaInfo(request.getMetaInfo(), resolvedAgentKey, scenario));
        entity.activate();
    ...
    private SessionTurnEntity createTurn(Long sessionId, String message) {
