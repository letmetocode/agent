...
flowchart TD
    ...
    G --> H[展开 nodes/edges 生成 agent_tasks(PENDING)]
    H --> I[保存 tasks]
...
- 逻辑：
  ...
  - `MCP_FUNCTION`：读取 `tool_config.mcpServerConfig`（或 `serverConfig/server`），构建 `ToolDefinition` + `McpToolCallback`。
...
**2.5 MCP 客户端管理**
- 传输方式：`transport` 显式指定；否则按 `command/sseUrl/url` 推断；`auto` 先尝试 Streamable HTTP，404/405 回落到 SSE。
- stdio：`command/args/env/workDir` 启动进程；JSON-RPC 2.0 帧（`Content-Length`）。
- Streamable HTTP：POST JSON，`Accept: application/json, text/event-stream`，自动携带 `Mcp-Session-Id`。
- SSE：先连 SSE 获取 `endpoint` 事件，再 POST 请求并等待 `message` 响应。
