...
- `TaskExecutor` 的 claim 配额、超时重试、提示词构造、输出判定、Critic 回滚与 Agent 选择规则分别下沉至 `TaskDispatchDomainService`、`TaskExecutionDomainService`、`TaskPromptDomainService`、`TaskEvaluationDomainService`、`TaskRecoveryDomainService`、`TaskAgentSelectionDomainService`。
