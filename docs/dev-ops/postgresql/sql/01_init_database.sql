...
-- 8. 质量评估事件表
-- =====================================================
CREATE TABLE IF NOT EXISTS quality_evaluation_events (
    id                  BIGSERIAL PRIMARY KEY,
    plan_id             BIGINT NOT NULL, -- 逻辑关联: agent_plans.id
    task_id             BIGINT NOT NULL, -- 逻辑关联: agent_tasks.id
    execution_id        BIGINT,          -- 逻辑关联: task_executions.id

    evaluator_type      VARCHAR(32) NOT NULL, -- WORKER_VALIDATOR / CRITIC
...
CREATE INDEX IF NOT EXISTS idx_quality_eval_plan_created
    ON quality_evaluation_events(plan_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_quality_eval_experiment_variant
    ON quality_evaluation_events(experiment_key, experiment_variant, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_quality_eval_task_execution
    ON quality_evaluation_events(task_id, execution_id);
