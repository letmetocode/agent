...
产品外显形态保持“对话式问答”，内核升级为“可执行任务引擎”，支持分步执行、工具调用、结构化输出、失败归因与续跑。
...
- Workflow Graph 运行时引擎的最终实现细节与选型定稿。
...
- 可演进性：Workflow Graph 运行时方案可在后续评审中替换，不阻断本阶段迭代。
...
| --- | --- | --- | --- | --- |
| 登录能力 | 本地账号密码登录闭环（login/logout/me） | 已补齐最小登录接口与登录页，鉴权拦截尚未全链路收口 | 中 | 第二阶段补齐统一鉴权校验与过期处理 |
| 角色模型 | 单账号承担使用+治理双职责 | 单账号模型已落地，治理动作审计粒度仍需增强 | 中 | 与鉴权治理一并收口 |
| 验收体系 | P0 场景 + 量化阈值 | 现有偏实现验收，阈值体系不完整 | 中 | 建立统一验收模板并纳入回归 |
| Workflow Graph（历史术语：SOP/DRG） | 仅冻结边界，后续专题定稿 | 当前存在草稿实现，不作为最终方案 | 中 | 专题评审后回填设计文档并更新 PRD |

## 11) Workflow Graph 运行时未决议题（后续专题）

当前仅冻结“治理源数据为 SOP Spec、运行时为 Workflow Graph 编译产物”的边界，不在本文件内锁定最终引擎细节。
后续将单独评审并确定 Workflow Graph 运行时方案，完成后同步更新：
...
- 2026-02-14：补齐最小登录实现状态，更新现状偏差与收敛计划描述。
- 2026-02-15：统一 Workflow Graph 术语，明确“对话式外显形态 + 可执行任务内核”定位；知识库检索测试占位接口下线，待真实召回链路接入后恢复。
