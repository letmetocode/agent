...
- DDD 充血收口：会话策略/终态汇总/Plan 聚合迁移/Task 执行策略与提示词/判定/回滚/Agent 选择策略下沉至 domain service。
