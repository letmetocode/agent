...
- 工具页：表格主视图 + 状态标识 + 详情抽屉
- 知识库页：列表 + 文档明细（检索测试接口已下线，待真实召回链路接入后再恢复）
...
- 已接入全局快捷键体系：`mod+k`、`mod+shift+n`、`?`、`esc`。
- 已提供快捷键帮助弹窗（页面内可见矩阵说明，`?` 打开、`esc` 关闭）。
- 已接入可达性播报（`aria-live`）与焦点管理（抽屉/弹层关闭后焦点恢复）。
- 已补齐主链路页面快捷键：
  - 会话页：`mod+enter`、`mod+shift+h`、`mod+shift+p`、`[`、`]`、`r`、`esc`。
  - 日志页：`f`、`mod+enter`、`esc`。
  - 任务页：`f`、`mod+enter`、`mod+shift+v`。
  - 任务详情页：`r`、`b`。
